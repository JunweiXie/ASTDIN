...
parser.add_argument("--proxies", type=int, default=config['model']['proxies'])
...
parser.add_argument("--is_lr", default=config['train']['is_lr'],type=eval)
...
def main():
    ...
    engine = trainer(device=device, scaler=scaler, lrate=args.learning_rate, num_nodes=args.num_nodes,
                     ...
                     memory_size=args.memory_size,hidden_size=args.hidden_size,lr_decay_step=args.lr_decay_step,
                     lr_decay_rate=args.lr_decay_rate,is_lr=args.is_lr,proxies=args.proxies
                     )
    ...
    for i in range(1, args.epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            tloss = engine.train(trainx, trainy, i)
            train_loss += tloss
        if args.is_lr:
        ...
        log = 'Epoch: {:03d}, Prediction Loss: {:.4f}, ' \
              ' Valid Loss: {:.4f}, LR: {:.8f},  Training Time: {:.4f}/epoch'
        print(log.format(i, mtrain_loss, mvalid_loss, lr, (t2 - t1)), flush=True)
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            output = engine.model(data,supports[0])
            y_true.append(label)
